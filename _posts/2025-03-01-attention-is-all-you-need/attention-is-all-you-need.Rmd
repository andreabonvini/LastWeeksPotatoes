...
$$
\color{darkgreen}{V^{'}}=\text{Attention}(\color{purple}Q,\color{darkorange}K,\color{darkgreen}V) = \text{softmax}\left({\frac{\color{purple}Q\color{darkorange}K^T}{\sqrt{d_k}}}\right)\color{darkgreen}V 
...
After normalizing the values by $\sqrt{d_k}$ and taking the $\text{softmax}$ we see that the new value vectors stored in $\color{darkgreen}{V^{'}}$ will be the result of a weighted sum of the old ones.

$$
\color{darkgreen}{V^{'}} = \text{softmax}\left({\frac{\color{purple}Q\color{darkorange}K^T}{\sqrt{d_k}}}\right)\color{darkgreen}V =\begin{bmatrix}
0.40 && 
0.10 && 
0.10 && 
0.20 && 
...
And, as anticipated, we may have that the old value vector for the word "weather" $\mathbf{V}^T_{\text{(weather)}}$ will have the chance to influence the new value vector for the word "cool" $\mathbf{V^{'}}^T_{\text{(cool)}}$.
...
Since the dot products that contain any "\<PAD\>" token shouldn't influence the final $\color{green}V$alue vectors we add a padding mask $M_{\text{pad}}$ to ensure padded tokens don't influence the final attention matrix:
...
The final $\color{green}V$alue update with padding mask applied will then look like this:

$$
\color{darkgreen}{V^{'}} = \text{softmax}\left({\frac{\color{purple}Q\color{darkorange}K^T}{\sqrt{d_k}}+M_\text{pad}}\right)\color{darkgreen}V =
$$
...
$$
\text{softmax}(z_i) = \frac{e^{z_i}}{\sum_{j=1}^{n_e}e^{z_j}} 
$$
...
The final $\color{green}V$alue update with both masks applied will then look like:

$$
\color{darkgreen}{V^{'}} = \text{softmax}\left({\frac{\color{purple}Q\color{darkorange}K^T}{\sqrt{d_k}}+M_\text{combined}}\right)\color{darkgreen}V
$$
...
The final $\color{green}V$alue update in cross-attention with padding masks applied will then look like:
